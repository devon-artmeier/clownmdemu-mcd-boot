...
UpdateObjects:
	...
	move.l	a1,object_sprite_slot				; Set sprite slot address
	move.w	#1,object_sprite_link				; Reset sprite link
...
.Update:
	...
	movea.l	object_index,a1					; Run object code
	movea.l	(a1,d0.w),a1
	jsr	(a1)
	
	move.l	obj.x_speed(a0),d0				; Move object
	add.l	d0,obj.x(a0)
	...
	movea.l	object_sprite_slot,a2				; Draw object
	move.w	object_sprite_link,d6
	bsr.s	DrawObjectSprite
	move.l	a2,object_sprite_slot
	move.w	d6,object_sprite_link
...
.NextObject:
	...
	movea.l	object_sprite_slot,a2				; Get current sprite slot
...
DrawObjectSprite:
	btst	#1,obj.sprite_flags(a0)				; Should this object be drawn?
	bne.s	.End						; If not, branch
	
	movea.l	obj.sprite(a0),a1				; Get sprite data
	
	moveq	#0,d1						; Get number of sprite pieces
	move.b	(a1)+,d1
	
	move.b	(a1)+,obj.sprite_flip(a0)			; Get flag that is affected by flipping
	tst.b	obj.sprite_flags(a0)
	bpl.s	.GetPosition
	addq.b	#1,obj.sprite_flip(a0)
	
.GetPosition:
	move.w	obj.y(a0),d2					; Get Y position
	move.w	obj.x(a0),d3					; Get X position
...
.DrawSprite:
	...
	or.b	obj.sprite_tile(a0),d0
	move.b	d0,(a2)+
	move.b	(a1)+,(a2)+
	
	move.b	(a1)+,d0					; Get piece X position
	tst.b	obj.sprite_flags(a0)				; Is the object's sprite flipped?
